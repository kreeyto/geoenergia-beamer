...
\newcommand{\definethemeassets}{%
	...
	% includes
	\newcommand{\backgroundfile}{stdbackground.png}%
	\newcommand{\udescfile}{udescwhite.png}%
	\newcommand{\geofile}{geowhite.png}%
	\newcommand{\petfile}{petwhite.png}%
	% cores footline
	...
	% includes
	\newcommand{\backgroundfile}{altbackground.png}%
	\newcommand{\udescfile}{udescgray.png}%
	\newcommand{\geofile}{geogray.png}%
	\newcommand{\petfile}{petgray.png}%
	% cores footline
...
\setbeamertemplate{footline}{
	\begin{tikzpicture}[remember picture, overlay]
		...
		\node[align=center,font=\scriptsize,white] at ([xshift=1cm,yshift=0.33cm]current page.south) {\presentername. \presentationfootline};
...
{\unspace} % nada antes
{\textsuperscript{\printfield{labelnumber}}} % número sobrescrito
{\multicitedelim} % separador (vírgula etc.)
{}

\NewDocumentCommand{\reference}{mm}{
	\hspace{-0.075cm}\stepcounter{refcount}%
	\edef\tempcounter{\number\value{refcount}}%
	\g@addto@macro\refentries{\item\hspace{-0.1cm}#2}%
	\cite{#1}%
}

\let\oldframe\frame
\renewenvironment{frame}{
	\setcounter{refstart}{\numexpr\value{refcount}+1\relax} % marca início real
	\gdef\refentries{}
	\oldframe
}{\endframe}
\makeatother
	...
	\begin{tikzpicture}[remember picture, overlay]
		\node[anchor=south west] at ([xshift=0.45cm,yshift=0.73cm]current page.south west) {
			...
			\begin{minipage}{\textwidth}
				\begin{enumerate}[label=$^\arabic*$,start=\value{refstart}]
					\refentries
				\end{enumerate}
			\end{minipage}
		};
...
\newcommand{\presentationfootline}{}
...
\newcommand{\setpresentationfootline}[1]{\renewcommand{\presentationfootline}{#1}}
...
\newcommand{\sectionframe}[2]{%
	\begin{frame}[plain]
		\begin{tikzpicture}[remember picture, overlay]
			...
			\node[anchor=east,align=right,text width=0.55\paperwidth] 
			at ([xshift=-0.5cm, yshift=-1cm]current page.east) {
				{\fontsize{16pt}{16pt}\bfseries \textcolor{\txttitle}{#1}} \\[1em]
				{\fontsize{12pt}{12pt}\bfseries \textcolor{\txtmisc}{#2}} 
			};
			
			\node[align=center,font=\scriptsize,\txtmisc] 
			at ([xshift=1cm,yshift=0.33cm]current page.south) 
			{\presentername. \presentationfootline};
			
			% número da página (estilo footline)
...
\newcommand{\thanksframe}[3]{%
	\begin{frame}[plain]
		\begin{tikzpicture}[remember picture, overlay]
			...
			% logos 
			\node[anchor=east,scale=0.97] at ([xshift=0.45cm, yshift=3cm]current page.east) {
				\begin{tikzpicture}[remember picture]
					\node[anchor=west,yshift=-0.05cm] at (-4.38,0) {\includegraphics[width=3.75cm]{\geofile}};
					\node[anchor=west] at (-1,0) {\includegraphics[width=3cm]{\udescfile}};
					\node[anchor=west,align=left,scale=0.66,text width=8cm,text=\txtdep,font=\bfseries,yshift=-0.05cm] at (2.2, 0) {\includegraphics[width=7cm]{\petfile}};
				\end{tikzpicture}
			};
			
			\node[anchor=east,align=right,text width=0.55\paperwidth] 
			at ([xshift=-0.5cm, yshift=-1cm]current page.east) {
				{\fontsize{16pt}{16pt}\bfseries \textcolor{\txttitle}{#1}} \\[1.5em]
				{\fontsize{12pt}{12pt}\bfseries \textcolor{\txtmisc}{#2}} \\[0.1em]
				{\fontsize{10pt}{10pt}\selectfont \textcolor{\txtmisc}{#3}}
			};
